      ...
      <div className="bg-slate-900 border border-slate-700 rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-slate-900 border-b border-slate-800 px-5 py-4 flex items-center justify-between">
          <div>
            ...
            <div className="text-xs text-slate-400">{match.league.round} · {match.fixture.venue.name}</div>
          </div>
          <button
            onClick={onClose}
        ...
        {/* Score */}
        <div className="px-5 py-6 text-center bg-slate-800/40">
          <div className="flex items-center justify-center gap-6">
            <div className="text-center">
              <div className="text-4xl mb-1">{hFlag}</div>
              <div className="font-display font-bold text-white text-sm">{match.teams.home.name}</div>
            </div>
            <div className="text-center">
              <div className={cn(
            ...
            <div className="text-center">
              ...
              <div className="font-display font-bold text-white text-sm">{match.teams.away.name}</div>
            </div>
          </div>
        ...
        {detail.loading ? (
          <div className="px-5 py-8 text-center text-slate-400 text-sm">Memuat statistik...</div>
        ) : (
          <>
            {/* Events */}
            {detail.events.length > 0 && (
              <div className="px-5 py-4">
                <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Jalannya Pertandingan</div>
                  ...
                  {detail.events.map((ev, i) => {
                    ...
                    return (
                      <div key={i} className={cn('flex items-center gap-2 text-sm', !isHome && 'flex-row-reverse')}>
                        <div className={cn('w-8 h-8 rounded-lg flex items-center justify-center text-base shrink-0', isHome ? 'bg-red-950' : 'bg-blue-950')}>
                          {icon}
                        </div>
                        <div className={cn('flex-1', !isHome && 'text-right')}>
                          <span className="font-semibold text-white">{ev.player.name}</span>
                          {ev.assist.name && <span className="text-slate-400"> (assist: {ev.assist.name})</span>}
                        </div>
                        <div className="font-display font-bold text-amber-400 w-8 text-center shrink-0">
                          {ev.time.elapsed}'
                        </div>
                      </div>
                    )
            ...
            {hStats.length > 0 && (
              <div className="px-5 pb-5">
                <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3">Statistik</div>
                {statKeys.map(key => (
